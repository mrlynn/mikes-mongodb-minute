...
export async function generateThumbnail({
  ...
  try {
    ...
    let faceBuffer = null;
    if (faceImagePath && existsSync(faceImagePath)) {
      const faceImage = await readFile(faceImagePath);
...
export async function saveThumbnail(episodeId, thumbnailBuffers) {
  ...
  const mainPath = join(thumbnailsDir, mainFilename);
  const smallPath = join(thumbnailsDir, smallFilename);

  await writeFile(mainPath, thumbnailBuffers.main);
  await writeFile(smallPath, thumbnailBuffers.small);

  return {
    mainUrl: `/uploads/thumbnails/${mainFilename}`,
    smallUrl: `/uploads/thumbnails/${smallFilename}`,
  };
}
